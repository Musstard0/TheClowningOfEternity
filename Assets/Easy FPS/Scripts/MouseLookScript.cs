...
void FixedUpdate(){
	...
	else if(GetComponent<PlayerMovementScript>().walkingSpeed > 5){
...
void ApplyingStuff(){
	...
	WeaponRotation();
		...
		//zRotation = 0;
        if (Input.GetAxis("Horizontal") < 0 && maxZRotation > zRotation)
        {
            zRotation += speedZRotation;
        }
        else if (Input.GetAxis("Horizontal") > 0 && -maxZRotation < zRotation)
        {
            zRotation -= speedZRotation;
        }
